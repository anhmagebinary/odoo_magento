...
class ConnectorSnippet(models.TransientModel):
    ...
    def sync_operations(self, model, mapping_model, domain, oprmodel):
        ...
        if type(connection) is dict and connection.get('status', 'False'):
            ...
            if record_objs:
                display_message = ''
                exup_record_objs, already_exup, not_exp = self.get_map_updt_objs(record_objs, instance_id, sync_opr, mapping_model)
                if exup_record_objs:
                ...
                if not_exp:
                    display_message += 'Information!\nListed {}(s) has not yet exported.'.format(oprmodel)
            else:
    ...
    def get_map_updt_objs(self, record_objs, instance_id, sync_opr, model):
        mapped_objs = self.env[model].search([('instance_id', '=', instance_id),('name','in',record_objs.ids)])
    ...
    def manual_connector_order_operation(self, opr, ecomm, sale_order, opr_obj=False):
        ...
        if order_map_obj:
            ...
            if opr_obj:
                opr_obj.write({f'ecomm_{opr}':response.get('ecomm_order_status_response','')})
        return response
        ...
        try:
            ...
            for instanceObj in mappings.mapped('instance_id'):
                ...
                if hasattr(self, 'delete_%s_mappings' % channel):
                    getattr(self, 'delete_%s_mappings' % channel)(ecomm_ids, ecomm_model, connection, odoo_ids)
        except Exception as e:
