...
class ConnectorInstance(models.Model):
    ...
    token = fields.Char(string='Integration/Auth Token')
    ...
    authentication_method = fields.Selection(
        ...
        ('admin', 'Admin Token')],
        default='admin',
        string='Magento Authentication Method')

    def test_magento2_connection(self):
        token = 0
        connection_status = False
        status = 'Magento Connection Un-successful'
        text = 'Test connection Un-successful please check the magento {} !!!'.format("login credentials" if self.authentication_method == 'admin' else "integration access token")
    ...
    def create_magento2_connection(self, vals={}):
        ...
        try:
            ...
            pass
        try:
            if self.authentication_method == 'admin':
                ...
                url = self.name + "/index.php/rest/V1/store/storeViews"
                response_api =requests.get(url, headers={"Accept":"application/json", "Authorization": str(token)})
            response = json.loads(response_api.text)
            if response_api.ok :
                token = "Bearer " + response if self.authentication_method == 'admin' else self.token
                ...
                for connection_obj in active_connections:
                    ...
                    if magento_url == act or magento_url[:-1] == act:
                        return connection_obj.read(
                            ['language', 'category', 'warehouse_id', 'route_ids', 'tracking', 'invoice_policy'])[0]
            else:
                for connection_obj in active_connections:
                    return connection_obj.read(
                        ['language', 'category', 'warehouse_id', 'route_ids', 'tracking', 'invoice_policy'])[0]
