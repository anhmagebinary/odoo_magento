...
class ConnectorSnippet(models.TransientModel):
    ...
    def create_magento2_product_attribute(self, name, odoo_id, connection, ecomm_attribute_code):
    ...
    def create_magento2_product_attribute_value(self, ecomm_id, attribute_obj, ecomm_attribute_code, instance_id, connection):
        if connection.get('status', False) and ecomm_attribute_code:
            ...
            for value_obj in attribute_obj.value_ids:
                ...
                if not self.env['connector.option.mapping'].search([('odoo_id', '=', value_id), ('instance_id', '=', instance_id)], limit=1):
                    ...
                    self.create_magento2_attribute_value(url, token, ecomm_attribute_code, value_id, value_name, position)
            self.map_magento2_attribute_values(url, token, ecomm_attribute_code, attribute_value_data)
        return True

    @api.model
    def create_magento2_attribute_value(self, url, token, ecomm_attribute_code, value_id, name, position='0'):
        if token:
            name = name.strip()
            options_data = {"option": {
                "label": name,
    ...
    @api.model
    def map_magento2_attribute_values(self, url, token, ecomm_attribute_code, attribute_value_data):
        ...
        if type(option_response) is list:
            ...
            for mage_option in option_response:
                if mage_option.get('value') and mage_option.get('value').isdigit():
