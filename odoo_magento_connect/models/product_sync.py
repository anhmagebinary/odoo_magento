...
class ConnectorSnippet(models.TransientModel):
    ...
    def _create_product_attribute_option(self, wk_attr_line_objs):
        ...
        for type_obj in wk_attr_line_objs.filtered(lambda x: x.attribute_id.create_variant != 'no_variant'):
    ...
    def _get_magento2_product_qty(self, prod_obj, instance_id, stock_item_id=False, stock_id=1):
        ...
        mob_warehouse_id = self.env['connector.instance'].sudo().browse(instance_id).warehouse_id.id
        ctx['warehouse'] = mob_warehouse_id
        prod_obj = prod_obj.with_context(ctx)
        ...
        if vrnt_obj:
            ...
            if prod_response and prod_response.get('id'):
                ...
                self.create_odoo_connector_mapping(
                    ...
                    magento_stock_id=prod_response['extension_attributes']['stock_item'].get('item_id',0)
                )
    ...
    def _export_magento2_specific_template(self, exup_product_obj, instance_id, channel, connection):
        ...
        if status and exup_product_obj:
            ...
            if not exup_product_obj.product_variant_ids:
                ...
                createVariantAttr = [x for x in exup_product_obj.mapped('attribute_line_ids.attribute_id.create_variant') if x!='no_variant']
                if not wk_attr_line_objs or not createVariantAttr or exup_product_obj.product_variant_count==1:
                    ...
                    if not mage_set_id:
                        ...
                        get_product_data.update(
                            ...
                            sku= exup_product_obj.config_sku or 'Template sku %s' % template_id,
            ...
            if temp_obj and mage_prod_id:
                if temp_obj.product_variant_ids:
                    ...
                    for vrnt_obj in temp_obj.product_variant_ids:
                        ...
                        if prod_map_obj:
                            self._update_magento2_specific_product(prod_map_obj[0], url, token, channel, connection)
                else:
                ...
                exup_product_obj.need_sync = False
